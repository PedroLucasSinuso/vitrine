...
def create_product_source(db: Session) -> ProductSource:
    ...
    return AlterdataProductSource(get_alterdata_engine(db))


def create_transaction_source(db: Session) -> TransactionSource:
    from app.adapters.alterdata.transaction_source import AlterdataTransactionSource
    from app.adapters.alterdata.db import get_alterdata_engine

    return AlterdataTransactionSource(get_alterdata_engine(db))


def create_sync_engine_and_source(
    db: Session, pool_size: int = 1
    ...
    engine = get_alterdata_engine(db, pool_size=pool_size)
...
def run_sync_common(
    ...
    """Executa o sync (engine + service) com lifecycle seguro.

    Cria engine PostgreSQL, instancia SyncService, executa sync,
    invalida cache de transações (opcional) e descarta o engine.
    ...
    Retorna SyncResult ou None em caso de falha.
    """
    from app.adapters.alterdata.db import get_alterdata_engine
    from app.adapters.alterdata.product_source import AlterdataProductSource
    ...
    engine = None
    try:
        engine = get_alterdata_engine(session, pool_size=pool_size)
        source = AlterdataProductSource(engine)
